<x-app-layout>
    <div class="flex bg-gray-150 min-h-screen">
        ...
        <div class="flex-1 ml-64 p-8">
            <h2 class="text-2xl font-semibold text-gray-800 mb-6">Create New Pet Breed</h2>
            ...
            <div class="bg-white shadow-lg rounded-lg p-6">
                <form action="{{ route('pet_breeds.store') }}" method="POST">
                    @csrf
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="breed" id="breed" class="w-full p-3 border border-gray-300 rounded-md" required>
                        @error('breed')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>

                    <button type="submit" class="bg-teal-600 text-white px-6 py-3 rounded-md">Create</button>
                </form>
            </div>
        </div>
    </div>
